    ...
    >
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          <Text style={styles.title}>{t("jellyseerr.request_button")}: {title}</Text>
          ...
          {mediaType === MediaType.TV && details?.seasons && (
            <View style={styles.seasonsContainer}>
              ...
              <View style={styles.seasonSelectionButtons}>
                ...
                >
                  <Text style={styles.buttonText}>{t("jellyseerr.select_all")}</Text>
                </Pressable>
                ...
                {details.seasons
                  ...
                  .map(season => (
                    <Pressable
                      ...
                      style={[
                        styles.seasonButton,
                        selectedSeasons.includes(season.seasonNumber) && styles.selectedSeason,
                        focusedSeason === `season-${season.seasonNumber}` && styles.focusedButton
                      ]}
                      onFocus={() => setFocusedSeason(`season-${season.seasonNumber}`)}
          ...
          <View style={styles.buttonContainer}>
            ...
            <Pressable
              style={[
                ...
                (mediaType === MediaType.TV && selectedSeasons.length === 0) && styles.disabledButton
              ]}
              onFocus={() => setFocusedButton('request')}
              onBlur={() => setFocusedButton(null)}
              onPress={handleRequest}
              disabled={mediaType === MediaType.TV && selectedSeasons.length === 0}
              hasTVPreferredFocus={true}
            >
              <Text style={styles.buttonText}>{t("jellyseerr.request_button")}</Text>
            </Pressable>
          </View>
        </View>
      </View>
