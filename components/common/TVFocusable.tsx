...
import { Platform, View, ViewProps, StyleSheet, TouchableOpacity, findNodeHandle } from 'react-native';
...
export const TVFocusable = forwardRef<any, TVFocusableProps>(({
  ...
  useEffect(() => {
    if (Platform.isTV && (hasTVPreferredFocus || forceFocus) && touchableRef.current) {
      // Give time for the component to fully mount
      const timer = setTimeout(() => {
        try {
          // @ts-ignore - requestTVFocus is not in the type definitions
          touchableRef.current?.setNativeProps?.({ hasTVPreferredFocus: true });
