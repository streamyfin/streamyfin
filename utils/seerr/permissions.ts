 ...
 *
 * The spec serves `permissions` as a plain number with nothing attached, so
 * this is one of the few things generation cannot give us. It is a bitmask
 * and one pure function, it has not moved in years, and the contract test
 * says the day a new bit appears.
 *
 * The only difference from the copy the app read through the submodule is a
 * rename: MANAGE_BLACKLIST and VIEW_BLACKLIST are MANAGE_BLOCKLIST and
 * VIEW_BLOCKLIST upstream, same values, and the app reads neither.
 */
