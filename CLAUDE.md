...
- **Translations**: When adding a translation key to a Text component, ensure the key exists in `translations/en.json` — it is the only source locale, every other catalogue is generated by Crowdin. Before adding new keys, check if an existing key already covers the use case.
