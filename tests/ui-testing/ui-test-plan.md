...
This v1 UI test setup uses Maestro to exercise the Expo/React Native Android phone, iOS phone, and Android TV login paths. The flows are written for an Android emulator/device or iOS simulator with Streamyfin installed and reachable Jellyfin credentials.

This machine does not need Android or iOS tooling to keep the files in the repo. Run the actual flows on a machine with the target emulator or simulator installed.
...
- `tests/ui-testing/flows/simple-flow.yaml` covers direct Jellyfin server login for Android phone and iOS phone.
- `tests/ui-testing/flows/simple-flow-dev.yaml` covers the same phone login steps after the app is already open on the server URL screen.
- `tests/ui-testing/flows/cf-flow.yaml` covers phone Jellyfin login through Cloudflare Access custom headers.
- `tests/ui-testing/flows/cf-flow-dev.yaml` covers the same phone Cloudflare login steps after the app is already open on the server URL screen.
