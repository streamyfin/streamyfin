...
import { flattenChannelItems, useChannelItems } from "@/hooks/useChannels";
import { useOrientation } from "@/hooks/useOrientation";
import * as ScreenOrientation from "@/packages/expo-screen-orientation";
import { apiAtom, userAtom } from "@/providers/JellyfinProvider";

const ChannelFolderPage: React.FC = () => {
  const searchParams = useLocalSearchParams();
  const { channelId, folderId } = searchParams as {
    channelId: string;
    folderId: string;
  };
  ...
  const { data: folder, isLoading: isFolderLoading } = useQuery({
    queryKey: ["channel-folder", folderId],
    queryFn: async () => {
      if (!api || !user?.Id || !folderId) return null;
  ...
  useEffect(() => {
    navigation.setOptions({ title: folder?.Name || t("channels.folder") });
  }, [navigation, folder, t]);
  ...
  if (isFolderLoading || isItemsLoading) {
    return (
